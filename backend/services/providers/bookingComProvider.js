...
class BookingComProvider extends TravelProviderInterface {
    constructor() {
        super();
        this.apiKey = process.env.BOOKING_COM_API_KEY || 'test'; // Use test key for now
    ...
    async searchHotels(cityCode, checkInDate, checkOutDate, adults = 1, radius = 5, maxResults = 20) {
        ...
        logger.info(`Booking.com: Returning mock hotel result for ${cityCode}`);
        return [
            {
                name: 'Test Booking.com Hotel',
                checkIn: checkInDate,
                ...
                provider: this.getProviderName(),
                cityCode,
                offers: [{ price: { total: 199.99, currency: 'USD' } }]
            }
        ];
    }
}

module.exports = BookingComProvider; 