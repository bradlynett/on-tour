...
async function createRealTripsWithFallback() {
  try {
    console.log('🔧 Creating trips with real data and intelligent fallback...');
    ...
    // Generate trips for each event using real data first
    for (const event of futureEventsResult.rows) {
      ...
      try {
        // Create trip using real data with fallback
    ...
    console.error('❌ Error creating real trips:', error);
  } finally {
    await pool.end();
  }
}
...
async function createTripWithRealData(event) {
  try {
    ...
    // 1. Ticket component - Try to get real data from Ticketmaster
    if (event.external_id) {
      ...
      try {
        // Try to get real ticket data from Ticketmaster
        const ticketData = await eventService.getEventDetails(event.external_id);
        if (ticketData && ticketData.priceRanges && ticketData.priceRanges.length > 0) {
          ...
          console.log(`   ✅ Got real ticket data: $${ticketPrice}`);
        } else {
        ...
        ticketDetails = {
          ...
          ticketType: 'Standard',
          delivery: 'Mobile Entry',
          note: 'Price estimated from event data'
        };
      }
    ...
    // 2. Flight component - Try real Amadeus data first
    if (destinationAirport && destinationAirport !== 'DEN') {
      ...
      try {
        ...
        if (flightData && flightData.length > 0) {
          ...
          console.log(`   ✅ Got real flight data: $${flightPrice}`);
        } else {
    ...
    // 3. Hotel component - Try real Amadeus data first
    if (event.venue_city) {
      ...
      try {
        ...
        if (hotelData && hotelData.length > 0) {
          ...
          console.log(`   ✅ Got real hotel data: $${hotelPrice}`);
        } else {
